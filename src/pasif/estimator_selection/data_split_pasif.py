...
class DataSplittingByPasif:
    ...
    def set_params(
        ...
        for action in range(self.batch_bandit_feedback['n_actions']):
            self.pscore_by_pi_e[self.batch_bandit_feedback['action'] == action] = self.action_dist_by_pi_e[
                self.batch_bandit_feedback['action'] == action, action, 0]

    def train_importance_fitting(self, process_id):
        """
        Train NN by importance fitting
        """
        if torch.cuda.is_available():
            num_of_gpus = torch.cuda.device_count()
            gpu_id = str(int(process_id % num_of_gpus))
            print('N° of GPUs:', num_of_gpus)
            print('GPU id:', gpu_id)
            device = torch.device('cuda:' + gpu_id)
            #print('Memory allocated before:', torch.cuda.memory_allocated(device))
            #print('Memory reserved before:', torch.cuda.memory_reserved(device))
            #print('Check data:', self.batch_bandit_feedback['action'][0:10])
        elif torch.backends.mps.is_available():
            device = torch.device('mps')
        else:
            device = 'cpu'
        print('Device type:', device)
        print('Batch bandit feedback rounds:', self.batch_bandit_feedback['n_rounds'])

        # convert data to Tensor type
        x_and_a = np.concatenate([
            self.batch_bandit_feedback['context'],
            self.batch_bandit_feedback['action'].reshape(self.batch_bandit_feedback['action'].shape[0], 1)], axis=-1)
        ...
        pi_b = self.batch_bandit_feedback['pi_b']
        if self.batch_bandit_feedback['position'] is not None:
            pi_b = np.zeros((self.batch_bandit_feedback['n_rounds'], self.batch_bandit_feedback['n_actions']))
            for idx in range(self.batch_bandit_feedback['position'].shape[0]):
                pi_b[idx, :] = self.batch_bandit_feedback['pi_b'][idx, :, self.batch_bandit_feedback['position'][idx]]

        pi_b = torch.from_numpy(pi_b).float()  # behavior policy

        pscore = np.array(self.batch_bandit_feedback['pscore'])
        pscore = torch.from_numpy(pscore).float()  # propensity score
        ...
        class CustomLoss(nn.Module):
            ...
            def forward(self, _outputs, _targets):
                ...
                marginal_p_ = 0  # marginal_p = E[rho(a|x)] under pi_b, --> marginal_p.shape = (1, n_batches)
                for action_ in range(1, 1 + n_actions):
                    marginal_p_ += _targets[:, action_] * _outputs[:, action_]
                tilde_p_e = _targets[:, -1] * (_outputs[:, 0] / marginal_p_)
                tilde_p_b = _targets[:, -1] * ((1.0 - _outputs[:, 0]) / (1.0 - marginal_p_))
                tilde_w = tilde_p_e / tilde_p_b  # importance weight by NN
                self.loss_d = ((tilde_w - _targets[:, 0]) ** 2).mean()  # _targets[:, 0] is the importance weight vector
                self.loss_r = ((marginal_p_ - k) ** 2).mean()
                loss_ = self.loss_d + regularization_weight * self.loss_r
                return loss_

        criterion = CustomLoss()

        # dim of context+action
        dim_context_and_action = self.batch_bandit_feedback['context'].shape[1] + 1
        ...
        class Net(nn.Module):
            ...
            def __init__(self):
                ...
                self.fc1 = nn.Linear(dim_context_and_action, 100)
        ...
        for epoch in range(self.n_epochs):
            # for phase in ['train', 'val']: # To cut the calculation time, we only conduct train phase
            # (There is no problem because all the data is used for training)
            for phase in ['train']:
                ...
                for t_x_and_a, t_w, t_pi_b, t_p in dataloader:
                    t_x_and_a, t_w, t_pi_b, t_p = t_x_and_a.to(device), t_w.to(device), t_pi_b.to(device), t_p.to(
                        device)
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        # t_x_and_a.shape = (n_batches, dim_context + 1)
                        # output_actual.shape = (n_batches, 1)
                        ...
                        # outputs.shape = (n_batches, 1 + n_actions) ----> 1 is the output_actual
                        # outputs = [rho(x_batch[0], a_batch[0]),                   rho(x_batch[0], 1),         ..., rho(x_batch[0], n_actions),
                        #            rho(x_batch[1], a_batch[1]),                   rho(x_batch[1], 1),         ..., rho(x_batch[1], n_actions),
                        #                                           ...
                        #            rho(x_batch[n_batches], a_batch[n_batches]),   rho(x_batch[n_batches], 1), ..., rho(x_batch[n_batches], n_actions)]
                        ...
                        # targets.shape = (1 + n_actions + 1, n_batches)
                        targets = torch.cat([t_w, t_pi_b, t_p], axis=1)

                        # calc loss
                        loss = criterion(_outputs=outputs, _targets=targets)
                ...
                if epoch % 5000 == 0 or epoch == 0:
                    print('Epoch {}/{}, {} TotalLoss: {:.10f} LossD: {:.10f} LossR: {:.10f}'.format(
                        epoch + 1, self.n_epochs, phase, epoch_loss, epoch_loss_d, epoch_loss_r))
        ...
        if np.isnan(self.split_info_dict['sampling_probability']).sum() > 0:
            ...
            for action in range(self.batch_bandit_feedback['n_actions']):
                self.split_info_dict['pi_b1'][:, action, 0] = self.batch_bandit_feedback['pi_b'][:, action, 0] * (
                        output_fixed_action[:, action] / self.split_info_dict['marginal_p'])
            ...
            for action in range(self.batch_bandit_feedback['n_actions']):
                self.split_info_dict['pi_b2'][:, action, 0] = self.batch_bandit_feedback['pi_b'][:, action, 0] * (
                        (1.0 - output_fixed_action[:, action]) / (1.0 - self.split_info_dict['marginal_p']))
            ...
            if sampling_indicator.sum() < 1:
                ...
                for feedback_key in self.batch_bandit_feedback.keys():
                    if feedback_key == 'n_rounds':
                        ...
                        self.batch_feedback_1[feedback_key] = self.batch_bandit_feedback[feedback_key][
                                                              self.split_info_dict['sampling_indicator'] == 1, :].copy()
                        ...
                        if self.batch_bandit_feedback[feedback_key] is None:
                            ...
                            self.batch_feedback_1[feedback_key] = self.batch_bandit_feedback[feedback_key][
                                                                  self.split_info_dict['sampling_indicator'] == 1,
                                                                  :].copy()
                    elif feedback_key == 'pi_b':
                        if self.batch_bandit_feedback[feedback_key] is None:
                            self.batch_feedback_1[feedback_key] = None
                        else:
                            self.batch_feedback_1[feedback_key] = self.split_info_dict['pi_b1'][
                                                                  self.split_info_dict['sampling_indicator'] == 1, :,
                                                                  :].copy()
                    elif feedback_key == 'pscore':
                        self.batch_feedback_1[feedback_key] = self.split_info_dict['pi_b1_pscore'][
                            self.split_info_dict['sampling_indicator'] == 1].copy()

                self.batch_feedback_2 = {}
                for feedback_key in self.batch_bandit_feedback.keys():
                    if feedback_key == 'n_rounds':
                        self.batch_feedback_2[feedback_key] = self.batch_bandit_feedback['n_rounds'] - sum(
                            self.split_info_dict['sampling_indicator'])
                    elif feedback_key == 'n_actions':
                        self.batch_feedback_2[feedback_key] = self.batch_bandit_feedback[feedback_key]
                    elif feedback_key == 'context':
                        self.batch_feedback_2[feedback_key] = self.batch_bandit_feedback[feedback_key][
                                                              self.split_info_dict['sampling_indicator'] == 0, :].copy()
                    elif feedback_key == 'action_context':
                        if self.batch_bandit_feedback[feedback_key] is None:
                        ...
                        if self.batch_bandit_feedback[feedback_key] is None:
                            ...
                            self.batch_feedback_2[feedback_key] = self.batch_bandit_feedback[feedback_key][
                                                                  self.split_info_dict['sampling_indicator'] == 0,
                                                                  :].copy()
                    elif feedback_key == 'pi_b':
                        if self.batch_bandit_feedback[feedback_key] is None:
                            self.batch_feedback_2[feedback_key] = None
                        else:
                            self.batch_feedback_2[feedback_key] = self.split_info_dict['pi_b2'][
                                                                  self.split_info_dict['sampling_indicator'] == 0, :,
                                                                  :].copy()
                    elif feedback_key == 'pscore':
                        self.batch_feedback_2[feedback_key] = self.split_info_dict['pi_b2_pscore'][
                            self.split_info_dict['sampling_indicator'] == 0].copy()
